...
function loadImage(cible){
      $('#draggable'+cible+' div img').load(function(){
          //$(this).parent().parent().fadeTo(700,1);
          numScroll++;
          EXIF.getData(this, function() {
              var make = EXIF.getTag(this, "Orientation");
              if(make=='3' && !window.mobilecheck()){
                var d = document.getElementById('img'+cible);
                d.className = d.className + " rotation3";
              }else if(make=='6' && !window.mobilecheck()){
                var d = document.getElementById('img'+cible);
                d.className = d.className + " rotation6";                  
              }else if(make=='8' && !window.mobilecheck()){
                var d = document.getElementById('img'+cible);
                d.className = d.className + " rotation8";                  
              }
          });     
          $(this).parent().parent().css('height',$(this).parent().parent().width()/1.33);       
          idHistoire--;
          console.log($('#draggable'+cible).width());
          //var wScreen = window.width;
          var wCible = ($('#draggable'+cible).width()-$('#draggable'+cible+' div img').width())/2;
          var hCible = ($('#draggable'+cible).height()-$('#draggable'+cible+' div img').height())/2;
          ...
          if(idHistoire>=0){
            recapHistoire(idHistoire,cible2,totalJour);
          }
      });    
}
 ...
 // $('.contain_Sujet').append('<div id="draggable'+cible+'" style="left:'+posX+';top:'+posY+';" class="blocHistoire sortable" alt="'+retourALT+'"><div class="dragImgClass"><img class="img'+cible+'" src="'+cible2[cible]+'" width="70" class="imgHistoire imgHistoire'+cible+'"></div></div>');
  ...
  if(cible2[cible].img!='' && cible2[cible].img!=undefined && cible2[cible].type!=numJour){
    ...
    if(cible2[cible].img.indexOf('icone_main')==-1 || cible2[cible].img.indexOf('icone_feuille')==-1){
      $('.contain_Sujet').append('<div id="draggable'+cible+'" class="blocHistoire sortable" alt="'+retourALT+'"><a href="uploadsDir/'+cible2[cible].img+'" class="dragImgClass"><img class="img'+cible+'" id="img'+cible+'" src="uploadsDir/'+cible2[cible].img+'"  exif="true" width="70" class="imgHistoire imgHistoire'+cible+'"></a><span class="titreImg">'+cible2[cible].mess+'<br>'+dateScreen+'</span></div>');                    
    }
  ...
  $('#draggable'+cible).bind('click',function(){
    ...
    /*$(this).css('width','95%');
    var scrollTop = $(this).offset().top-30;
...
$(document).ready(function(){
  ...
  socket.on('upload_notif', function (user) {
    //updateIcone('img107',"icone11");
      $('.contain_Sujet').prepend('<div id="draggable'+numSousSouvenir+'" class="blocHistoire" alt="'+numDrag+'"><div class="dragImgClass"><img class="img'+numDrag+'" src="'+user.img+'" width="70" class="imgHistoire imgHistoire'+numDrag+'"><span class="titreImg">'+user.mess+'<br>'+user.date+'</span></div></div>');
      numSousSouvenir++;
  });
